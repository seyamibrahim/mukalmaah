          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="grid gap-4 md:grid-cols-1">
              {message.hadiths.map((hadith, idx) => (
                <div
                  key={idx}
                  className="bg-surface border border-custom p-6 rounded-2xl glass-panel group shadow-sm transition-all hover:border-primary/30"
                >
                  {hadith.arabic && (
                    <p
                      className="text-xl md:text-2xl font-arabic text-right mb-5 leading-loose text-primary"
                      dir="rtl"
                    >
                      {hadith.arabic}
                    </p>
                  )}
                  <p className="text-sm md:text-base text-text mb-4 leading-relaxed border-l-4 border-primary pl-4 py-1 italic">
                    "{hadith.translation}"
                  </p>
                  <p className="text-xs text-muted font-medium bg-background inline-block px-3 py-1.5 rounded-md border border-custom">
                    {hadith.reference}
                  </p>
                </div>
              ))}
          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {message.duas.map((dua, idx) => (
                <div
                  key={idx}
                  className="bg-background border border-custom p-5 rounded-2xl glass-panel group"
                >
                  {dua.title && (
                  ...
                  {dua.arabic && (
                    <p
                      className="text-xl font-arabic text-right mb-4 leading-loose text-primary"
                      dir="rtl"
                    >
                      {dua.arabic}
                    </p>
